...
//	    	break;
	...
	public void historyWrite() {
		...
		HistoryVO hvo = new HistoryVO();		// 히스토리 데이터를 담을 VO 객체
		System.out.print("  1. 직원 번호 >> ");	// 하나씩 입력 받아서 
		hvo.setEmid(sc.next());					// HistoryVO에 있는 setter 이용하여 히스토리 테이블에 데이터 삽입
		System.out.print("  2. 현재 부서 >> ");
		hvo.setDname(sc.next());	
		System.out.print("  3. 시작 날짜 >> ");
		hvo.setLeaveStart(sc.next());	
		System.out.print("  4. 종료 날짜 >> ");
		hvo.setLeaveFin(sc.next());	
		System.out.print("  5. 현재 직급 >> ");
		hvo.setPosition(sc.next());	
		System.out.print("  6. 비고 >> ");
		hvo.setRemark(sc.next());	
		System.out.println();
		
		if(hdao.hisInsert(hvo)) {				// hdao에 있는 hisInsert 메서드 호출 (boolean 반환)
		System.out.println("--------------------------------");
	   	System.out.println("        등록이 완료되었습니다.       ");
	   	System.out.println("--------------------------------");
	   	emManage();								// 정상 등록되면 메시지 출력 후 직원정보 관리 메뉴로 돌아감
		} else {
			System.out.println("--------------------------------");
			System.out.println("         등록에 실패했습니다.       ");
		 	System.out.println("--------------------------------");
		 	historyWrite();						// 등록 안되면 메시지 출력 후 히스토리 등록 메뉴로 돌아감
		}
	}
	
	public void historyModify() {	// 히스토리 수정 메인 메서드
		System.out.println("  ╔═══════════════════════════╗"); 
		...
		System.out.println();
		System.out.println("  1. 부서   2. 직급   3. 시작일자");
		System.out.println("  4. 종료일자   5. 비고" );
		System.out.println();
		System.out.print("  💡 선택(숫자 입력) >> ");
		input = sc.nextInt();					// 수정할 내용의 번호 입력 HistoryDAO에서도 쓰기 위해 공유 변수 사용
		
		switch(input) {
		case 1:
			historyModify1(HrReadMain.no);		// 개별조회에 사용된 히스토리번호를 공유변수 no으로 가져와 사용
			break;
		case 2:
			historyModify2(HrReadMain.no);		// 번호에 따라 알맞은 수정 메서드로 이동
			break;
		case 3:
			historyModify3(HrReadMain.no);
			break;
		case 4:
			historyModify4(HrReadMain.no);
			break;
		case 5:
			historyModify5(HrReadMain.no);
			break;
		default :
			System.out.println();
			System.out.println("--------------------------------");
		   	...
		   	System.out.println("--------------------------------");
		   	System.out.println();
			historyModify();					// 다른 값 입력 시 메시지 출력 후 수정 메뉴로 돌아감
			
		}	// switch end
	   
	}
	
	public void historyModify1(int hno) {	// 히스토리 부서 수정
		System.out.println("  ╔═══════════════════════════╗"); 
		System.out.println("  ║        히스토리 수정      ║");
		System.out.println("  ╚═══════════════════════════╝");
		System.out.println();
		...
		HistoryVO hvo = hdao.hisSelect(hno);	// 수정하기 전 HistoryDAO에서 select한 데이터 hvo로 받기		
		System.out.print("  1. 현재 부서 >> ");
		hvo.setDname(sc.next());				// 수정할 내용 setter로 받기
		System.out.println();
	...
	public void historyModify2(int hno) {	// 히스토리 직급 수정
		System.out.println("  ╔═══════════════════════════╗");  
		System.out.println("  ║        히스토리 수정      ║");
		System.out.println("  ╚═══════════════════════════╝");
		System.out.println();
		...
		HistoryVO hvo = hdao.hisSelect(hno);	// 수정하기 전 HistoryDAO에서 select한 데이터 hvo로 받기	
		System.out.print("  2. 현재 직급 >> ");
		hvo.setPosition(sc.next());				// 수정할 내용 setter로 받기
		System.out.println();
	...
	public void historyModify3(int hno) {	// 히스토리 시작일자 수정
		System.out.println("  ╔═══════════════════════════╗"); 
		System.out.println("  ║        히스토리 수정      ║");
		System.out.println("  ╚═══════════════════════════╝");
		System.out.println();
	...
	public void historyModify4(int hno) {	// 히스토리 종료일자 수정
		System.out.println("  ╔═══════════════════════════╗");  
		System.out.println("  ║        히스토리 수정      ║");
		System.out.println("  ╚═══════════════════════════╝");
		System.out.println();
		...
		HistoryVO hvo = hdao.hisSelect(hno);	// 수정하기 전 HistoryDAO에서 select한 데이터 hvo로 받기	
		System.out.print("  4. 종료일자 >> ");
		hvo.setLeaveFin(sc.next());				// 수정할 내용 setter로 받기
		System.out.println();
	...
	public void historyModify5(int hno) {	// 히스토리 비고 수정
		System.out.println("  ╔═══════════════════════════╗");  
		System.out.println("  ║        히스토리 수정      ║");
		System.out.println("  ╚═══════════════════════════╝");
		System.out.println();
		...
		HistoryVO hvo = hdao.hisSelect(hno);	// 수정하기 전 HistoryDAO에서 select한 데이터 hvo로 받기		
		System.out.print("  5. 비고 >> ");
		hvo.setRemark(sc.next());				// 수정할 내용 setter로 받기
		System.out.println();
	...
	public void historyRemove(int hno) {	// 히스토리 삭제 
		System.out.println("  ╔═══════════════════════════╗");  // 2칸씩
		...
		if(input.equalsIgnoreCase("Y")) { 	// Y를 입력했을 때 delete 실행
			if(hdao.hisDelete(hno)) {		// delete 실행 후 true 반환 시 완료 메시지 출력 후 메인 메뉴로 돌아감
				...
				System.out.println("------------------------------------");
				emManage();
			} else {						// false 반환 시 실패 메시지 출력 후 다시 삭제 메뉴 출력
				System.out.println();
				System.out.println("--------------------------------");
		...
		} else if (input.equalsIgnoreCase("N")) {	// N을 입력했을 때 취소 메시지 출력 후 메인 메뉴로 돌아감
		   	System.out.println();
			System.out.println("------------------------------------");
			System.out.println("  취소되었습니다. 직원관리 메뉴로 돌아갑니다."); 
			System.out.println("------------------------------------");
		   	emManage();
	...
	public void workWrite() {	// 근태 기록 등록하는 메서드
		...
		System.out.print("  1.직원번호 >> ");
			wvo.setEmid(sc.next());				// 입력받은 값을 YearVO의 setter에 저장
		...
		//System.out.print("  8.잔여연차일수 >> "); // 잔여연차일수는 사용가능한 연차일수에서 사용한 연차일수 뺄셈 계산해서
			wvo.setUnusedAnn(avann - usedann);	// 입력하지 않아도 자동으로 테이블에 들어가게 함
		System.out.println();

		if(wdao.workInsert(wvo)) {				// wdao에 있는 workInsert 메서드 호출(boolean 반환)
			System.out.println("--------------------------------");
			System.out.println("        등록이 완료되었습니다.      ");
			System.out.println("--------------------------------");
			System.out.println();
			workManage();						// 정상 등록되면 관리 메뉴로 돌아감
		} else {
			System.out.println("--------------------------------");
			System.out.println("       정보 등록에 실패했습니다.     ");
			System.out.println("--------------------------------");
			System.out.println();
			workWrite();						// 등록 실패하면 등록화면 다시 출력
		}
	...
	public void workModify5(String emid) {	// 근태 기록 사용가능한 연차일수 수정 메서드
		...
		System.out.print("  5.사용가능 연차일수 >> ");
			...
			wvo.setUnusedAnn(avann - usedann);	// 사용가능한 연차일수 수정됨에 따라 잔여연차일수 자동으로 계산되어 수정
		System.out.println();
	...
	public void workModify6(String emid) {	// 근태 기록 사용연차일수 수정 메서드
		...
		System.out.print("  6.사용연차일수 >> ");
			...
			wvo.setUnusedAnn(avann - usedann); 	// 사용한 연차일수 수정됨에 따라 잔여연차일수 자동으로 계산되어 수정
		System.out.println();
	...
	public void yearWrite() {	// 연차 승인 내역 등록 메서드
		...
		System.out.println();
		YearVO yvo = new YearVO();				// 등록할 값을 담을 yvo 객체 생성
		System.out.print("  1.직원번호 >> ");
			yvo.setEmid(sc.next());				// 하나씩 입력한 값을 yvo의 setter에 저장
		...
		System.out.println();
			if(ydao.yearInsert(yvo)) {			// ydao에 있는 yearInsert 메서드 호출(boolean 반환)
				System.out.println("--------------------------------");
				System.out.println("       등록되었습니다.          ");
				System.out.println("--------------------------------");
				System.out.println();
				workManage();					// 정상 등록되면 근태 관리 메뉴로 돌아감
			} else {
				System.out.println("--------------------------------");
				System.out.println("     정보등록에 실패했습니다.   ");
				System.out.println("--------------------------------");
				System.out.println();
				yearWrite();					// 등록 실패하면 등록화면 다시 출력
			}
	...
	public void yearModify() {	// 개인의 연차 승인 내역 수정 메인 메뉴
		...
		input = sc.nextInt();				// 수정할 내용의 번호 입력 YearDAO에서도 쓰기 위해 공유 변수 사용
		switch (input) {
		case 1:				// 1 입력 시 연차사용날짜 수정
			yearModify1(HrReadMain.no);		// 개별조회에서 사용된 연차승인번호를 담은 공유변수 no 사용
			break;
	...
	public void yearModify1(int emno) {	// 개인의 연차 승인 내역 연차사용일자 수정 메서드
		...
		System.out.println();
		System.out.println("    < 수정할 내용을 입력해주세요 >  ");
		YearVO yvo = ydao.yearSelect(emno);		// YearDAO에 있는 select한 데이터를 yvo로 받기
		System.out.println();
		System.out.print("  1.연차사용일자 >> ");
			yvo.setYearDate(sc.next());			// 수정할 값을 yvo의 setter에 받기
		System.out.println();	
	...
	public void yearModify2(int emno) {	// 개인의 연차 승인 내역 수정 메서드
		...
		System.out.print("  2.연차사용일수 >> ");	
			yvo.setYearCnt(sc.next());			// 수정할 값을 yvo의 setter에 받기
	...
	public void yearModify3(int emno) {	// 개인의 연차 승인 내역 수정 메서드
		...
		System.out.print("  3.연차승인자 >> ");
			yvo.setApp(sc.next());				// 수정할 값을 yvo의 setter에 받기
		System.out.println();	
	...
	public void yearModify4(int emno) {	// 개인의 연차 승인 내역 수정 메서드
		...
		System.out.print("  4.연차승인일자 >> ");
			yvo.setAppDate(sc.next());			// 수정할 값을 yvo의 setter에 받기
		System.out.println();	
	...
	public void yearRemove(int emno) {	// 개인의 연차 승인 내역 삭제 메서드
		...
		if(input.equalsIgnoreCase("Y")) { 	// Y를 선택했을 때 delete 실행
			if(ydao.yeardelete(emno)) {		// delete 실행 후 true 반환 시 완료 메시지 출력 후 근태 관리 메뉴로 돌아감
				System.out.println();
				System.out.println("--------------------------------");
				System.out.println("           삭제되었습니다.          ");
				System.out.println("--------------------------------");
		...
		} else if (input.equalsIgnoreCase("N")) {	// N을 선택했을 때 취소 메시지 출력 후 근태 관리 메뉴로 돌아감
		   	System.out.println();
		   	System.out.println("--------------------------------");
		   	System.out.println("           취소되었습니다.          ");
		   	System.out.println("--------------------------------");
